...
def write_cbz_metadata(
    ...
    *,
    ...
    patched = patch_comicinfo(
        original,
        set_fields=set_fields,
        clear_fields=clear_fields,
        validation_profile=comicinfo_profile,
    )
    if comic_names:
    ...
    try:
        ...
        result = verify_cbz(
            source, staged, set_fields, clear_fields, comicinfo_profile=comicinfo_profile
        )
...
def verify_cbz(
    ...
    *,
    ...
    try:
        ...
        with zipfile.ZipFile(candidate) as archive:
            ...
            if len(names) != 1:
                ...
                candidate_comicinfo = archive.read(names[0])
                ...
                if comicinfo_profile == COMICINFO_PROFILE_IMAGEHASH:
                    source_hashes = _source_imagehash_snapshot(source)
                    if imagehash_snapshot(candidate_comicinfo) != source_hashes:
                        errors.append("ComicInfo Page ImageHash attributes changed")
    ...
    return VerificationResult(
        ...
        (
            "zip_crc",
            "payload_hashes_and_order",
            "comicinfo_schema",
            "comicinfo_profile",
            "imagehash_preservation",
            "metadata_readback",
        ),
        tuple(errors),
    )
...
def _source_imagehash_snapshot(path: Path) -> tuple[tuple[str | None, str], ...]:
    with zipfile.ZipFile(path) as archive:
        names = [
            name for name in archive.namelist() if name.casefold() == COMICINFO_NAME.casefold()
        ]
        if not names:
            return ()
        if len(names) != 1:
            raise ValueError("source must contain at most one ComicInfo.xml")
        return imagehash_snapshot(archive.read(names[0]))
